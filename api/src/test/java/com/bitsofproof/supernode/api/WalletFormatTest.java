...
public class WalletFormatTest
{
	...
	public void testInOut () throws ValidationException
	{
		...
		SerializedWallet read = SerializedWallet.readWallet (new ByteArrayInputStream (out.toByteArray ()), "passphrase");
		for ( int i = 0; i < 5; ++i )
		{
			assertTrue (read.getKeys ().get (i).key.equals (sw.getKeys ().get (i).key));
			ExtendedKey k = ExtendedKey.parse (read.getKeys ().get (i).key);
			for ( Token token : ScriptFormat.parse (read.getTransactions ().get (i).getOutputs ().get (0).getScript ()) )
			{
				if ( token.data != null )
				{
					byte[] address = k.getKey (i).getAddress ();
					assertTrue (Arrays.equals (token.data, address));
					assertTrue (read.getAddresses ().get ("coinbase" + i).equals (AddressConverter.toSatoshiStyle (address, 0x0)));
				}
			}
		}
	}
	...
	public void testStored () throws ValidationException
	{
		...
		SerializedWallet read = SerializedWallet.readWallet (in, "passphrase");
		for ( int i = 0; i < 5; ++i )
		{
			ExtendedKey k = ExtendedKey.parse (read.getKeys ().get (i).key);
			for ( Token token : ScriptFormat.parse (read.getTransactions ().get (i).getOutputs ().get (0).getScript ()) )

...
start() {
  ...
  if [ -z "$BOP_USER" -o `id -un` = "$BOP_USER" ] ; then
    nohup java ${JVM_FLAGS} -jar ${BOP_BASE}/server/target/bitsofproof-server-1.2.0.jar ${BOP_CONTEXTS} > /dev/null 2> /dev/null &
  else
    sudo -n -u ${BOP_USER} nohup java ${JVM_FLAGS} -jar ${BOP_BASE}/server/target/bitsofproof-server-1.2.0.jar ${BOP_CONTEXTS} > /dev/null 2> /dev/null &
  fi

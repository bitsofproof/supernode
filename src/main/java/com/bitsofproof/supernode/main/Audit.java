...
public class Audit
{
	...
	public void start (String[] args) throws IOException, TransactionValidationException
	{
		...
		gnuOptions.addOption ("u", "upto", true, "Run checks upto n");
		gnuOptions.addOption ("f", "from", true, "Run from check n");
		gnuOptions.addOption ("c", "check", true, "Run check n");
		...
		try
		{
			...
			if ( cl.hasOption ('u') )
			{
				int upto = Integer.valueOf (cl.getOptionValue ('u'));
				audit (1, upto);
				return;
			}
			if ( cl.hasOption ('c') )
			{
				int c = Integer.valueOf (cl.getOptionValue ('c'));
				audit (c, c);
				return;
			}
			if ( cl.hasOption ('f') )
			{
				int f = Integer.valueOf (cl.getOptionValue ('f'));
				audit (f, Integer.MAX_VALUE);
				return;
			}
			audit (1, Integer.MAX_VALUE);
		}
